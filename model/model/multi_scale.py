...
class MHA(nn.Module):
    ...
    def forward(self, x):
        ...
        self.attention_weights = torch.nn.functional.softmax(scaled_attention_logits, dim=-1)
...
class C(nn.Module):
    def __init__(self):     #block_size=10
        ...
        self.cnn = nn.Conv1d(1, 256, 3, padding=1)
        self.bn = nn.BatchNorm1d(256)
        self.pool = nn.AvgPool1d(kernel_size=4)


    def forward(self, data):    #(n, 1, 700)
        x = self.cnn(data)      #(n, 256, 700)
        x = self.bn(x)
        x = self.pool(x)        #(n, 256, 175)
