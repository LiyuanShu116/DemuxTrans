...
from model.WarpDemuX.WarpDemuX_main.warpdemux.parallel_distances import distance_matrix_to
...
class DTW_SVM_Model:
    ...
    def predict(
        ...
        # ===== 添加检查 SVM 决策函数输出的代码 =====
        # 获取决策分数，观察各类别的区分情况
        decision_values = self.model.decision_function(K)
        print("Decision function values (first 5 samples):")
        print(decision_values[:5])
        # 如果决策值整体接近 0，说明模型在各类别间的区分能力较低，
        # 这将导致通过 Platt Scaling 转换后的概率差异较小。
        # ==========================================
