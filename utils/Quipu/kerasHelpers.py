...
def resetHistory():
    ...
    # log files
    logs = glob.glob("/home/Logs/keras/*")
    for f in logs: 
        os.remove(f)
    subprocess.Popen(["tensorboard","--logdir=/home/Logs/keras"])
    tensorboard = TensorBoard(log_dir='/home/Logs/keras', write_graph=False, write_grads=False)
    history = History()
    epoch = 1
